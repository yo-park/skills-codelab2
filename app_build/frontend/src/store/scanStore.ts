...
export const useScanStore = create<ScanState>((set, get) => ({
  ...
  startScan: async () => {
    ...
    try {
      ...
      let progressBuffer: Record<number, ServerProgress> = {};
      let progressTimer: any = null;

      const flushProgress = () => {
        if (Object.keys(progressBuffer).length === 0) return;
        set(state => {
          let updatedProgress = false;
          const newFileProgress = { ...state.fileProgress };

          for (const [fileIndexStr, pData] of Object.entries(progressBuffer)) {
            const fileIndex = Number(fileIndexStr);
            const current = newFileProgress[fileIndex];
            if (current) {
              newFileProgress[fileIndex] = {
                ...current,
                bytesRead: pData.bytes_read,
                linesScanned: pData.lines_scanned,
                matchesFound: pData.matches_found
              };
          ...
          if (updatedProgress) {
            return { fileProgress: newFileProgress };
          }
          return state;
        });
        progressBuffer = {};
      ...
      eventSource.addEventListener('progress', (e: any) => {
        ...
        progressBuffer[data.file_index] = data;
