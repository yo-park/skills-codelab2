  ...
  startScan: async () => {
    ...
    try {
      ...
      eventSource.addEventListener('file_start', (e: any) => {
        ...
        // ⚡ Bolt: Using plain objects (Records) instead of ES6 Maps for high-frequency SSE updates.
        // Object spreading (...state) is significantly faster and creates less GC churn
        // than cloning a Map (new Map(state.fileProgress)) hundreds of times per second.
        set(state => ({
          fileProgress: {
            ...state.fileProgress,
            [data.file_index]: {
