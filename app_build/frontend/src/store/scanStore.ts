...
export const useScanStore = create<ScanState>((set, get) => ({
  ...
  startScan: async () => {
    ...
    try {
      ...
      // Buffers and timers for throttling high-frequency events
      ...
      const flushBuffers = () => {
        ...
        set(state => {
          let updates: Partial<ScanState> = {};

          if (Object.keys(progressBuffer).length > 0) {
            const newFileProgress = { ...state.fileProgress };
            let updated = false;

            for (const [idxStr, data] of Object.entries(progressBuffer)) {
              const idx = Number(idxStr);
              const current = newFileProgress[idx];
              if (current) {
                newFileProgress[idx] = {
                  ...current,
            ...
            if (updated) {
              updates.fileProgress = newFileProgress;
            }
            progressBuffer = {};
          }

          if (matchBuffer.length > 0) {
            updates.matches = [...state.matches, ...matchBuffer];
            matchBuffer = [];
          }
      ...
      const scheduleFlush = () => {
        if (!throttleTimer) {
          throttleTimer = setTimeout(flushBuffers, 100);
        }
      };
